...
def get_log_file_path_from_cmd_line(param_num):
    """Get the full path of the log file from the command line
...
def filter_log_by_regex(log_file, regex, ignore_case=True, print_summary=False, print_records=False):
    ...
    if print_summary is True:
        print(f'The log files contains {len(records)} records that case-{"in" if ignore_case else ""}sensitive match the regex "{regex}".')
